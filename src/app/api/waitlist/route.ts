...
const waitlistSchema = z.object({
  ...
  runsAds: z.string().trim().max(120).optional(),
  bookingSystem: z.string().trim().max(160).optional(),
...
export async function POST(request: Request) {
  try {
    ...
    const signupInput = {
      ...
      businessType: optionalValue(input.businessType) || "Garage door company",
    ...
    const metadata = {
      ...
      leadDetails: {
        serviceArea: optionalValue(input.serviceArea),
        runsAds: optionalValue(input.runsAds),
        bookingSystem: optionalValue(input.bookingSystem),
