...
const proofCards = [
  { icon: PhoneCall, value: "1.8s", label: "answer time", helper: "Bellory picks up before callers hang up." },
  { icon: CalendarCheck, value: "24/7", label: "booking coverage", helper: "After-hours calls become jobs, not voicemails." },
  { icon: BadgeDollarSign, value: "$7k+", label: "missed-call leak", helper: "Potential monthly revenue recovered per busy service shop." },
] as const;
...
function HeroConsole() {
  return (
    <motion.div
      ...
      className="relative"
    >
      <div className="absolute -inset-5 rounded-[2rem] bg-[#C7F76F]/10 blur-3xl" />
      <Card className="relative overflow-hidden p-4 sm:p-5">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_12%,rgba(199,247,111,.16),transparent_32%)]" />
        <div className="relative rounded-[1.25rem] border border-white/[.07] bg-[#15110C]/72 p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <span className="grid size-11 place-items-center rounded-2xl bg-[#C7F76F]/10 text-[#C7F76F]"><PhoneCall size={18} /></span>
              <div>
                <p className="text-[13px] font-black text-white">Incoming call</p>
                <p className="text-[11px] text-[#94836A]">Canyon HVAC - after hours</p>
              </div>
            </div>
            <GlowPill>Answered</GlowPill>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <div className="rounded-2xl border border-white/[.07] bg-white/[.035] p-4">
              <p className="text-[10px] font-black uppercase tracking-[.16em] text-[#C7F76F]">Bellory heard</p>
              <p className="mt-2 text-[13px] leading-6 text-[#FFF7E8]">“My AC is out and the house is 86 degrees. Can someone come tomorrow?”</p>
            </div>
            <div className="rounded-2xl border border-white/[.07] bg-white/[.035] p-4">
              <p className="text-[10px] font-black uppercase tracking-[.16em] text-[#F6C66A]">Bellory did</p>
              <div className="mt-3 space-y-2">
                {["Confirmed service area", "Checked booking rules", "Created appointment hold"].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-[12px] text-[#EDE0C8]"><Check size={13} className="text-[#C7F76F]" /> {item}</div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <VisualMetric icon={TimerReset} label="saved" value="8 min" />
            <VisualMetric icon={CalendarCheck} label="status" value="Held" tone="blue" />
            <VisualMetric icon={BadgeDollarSign} label="value" value="$420" tone="honey" />
          </div>
        </div>
      </Card>
    </motion.div>
...
export function LandingPage() {
  return (
    <main className="landing-page min-h-screen overflow-hidden text-[#FFF7E8]">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl gap-10 px-4 pb-16 pt-10 sm:px-6 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:pb-24 lg:pt-20">
        <div className="flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            <Badge><span className="pulse-ring size-1.5 rounded-full bg-[#C7F76F]" /> Built for businesses where every call can become revenue</Badge>
            <h1 className="mt-6 max-w-4xl text-5xl font-semibold leading-[.92] tracking-[-.075em] text-white sm:text-7xl lg:text-8xl">
              Never miss a call. Buy back your time.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#B7AB98]">
              Bellory answers like a polished receptionist, books from your calendar, protects your quote rules, and escalates the calls a human should handle.
            </p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16, duration: 0.55 }} className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {proofCards.map((card, index) => (
              <motion.div key={card.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 + index * 0.08, duration: 0.55 }} className="rounded-2xl border border-white/[.07] bg-white/[.035] p-4">
                <card.icon size={17} className="mb-4 text-[#C7F76F]" />
      ...
      <section id="proof" className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section id="how" className="relative z-10 mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl gap-5 px-4 py-12 sm:px-6 lg:grid-cols-[.92fr_1.08fr] lg:px-8">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl gap-6 px-4 py-16 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:px-8">
        <div className="flex flex-col justify-center">
          <Badge><Star size={12} /> Private launch</Badge>
      ...
      <footer className="relative z-10 border-t border-white/[.06] px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 text-[12px] text-[#94836A] md:flex-row md:items-center md:justify-between">
